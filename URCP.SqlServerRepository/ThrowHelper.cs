    ...
    internal static class ThrowHelper
    {
        public static RepositoryException ReThrow(Exception ex)
        {
            ...
            if (sqlEx != null)
            {
                ...
                return new RepositoryException(message, error, ex);
            }
            else if (ex is InvalidOperationException)
            {
                return new RepositoryException(ex.Message, ErrorTypeEnum.None, ex);
            }
            else if (ex is DbUpdateConcurrencyException)
            {
                return new RepositoryException("تم تعديل الكائن من قبل مستخدم آخر, يرجى تحديث المستعرض والمحاولة مرة آخرى", ex);
            }
            else if (ex is ModelValidationException)
            {
                return new RepositoryException("يجب التأكد من أن قواعد التأكد من الصحة صحيحة", ErrorTypeEnum.ValidationError, ex);
            }
