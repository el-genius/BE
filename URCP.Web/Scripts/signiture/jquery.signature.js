...
(function ($) { // Hide scope, no $ conflict
    ...
    var signatureOverrides = {
        ...
        options: {
            ...
            syncFieldImageURL: null, //added by Fadi Omar Selector for synchronised imageURL
            change: null, // Callback when signature changed
            required: false //added by Fadi Omar to validate signature
        },

        _validateSignature: function () {
            var valid = true;

            if (this.toJSON().length < 13)
                valid = false;
        ...
        _create: function () {
            ...
            try {
                ...
                if (this.options.required) {
                    $(this.canvas).closest('form').bind('submit', function () {
                        return self._validateSignature();
                    })
                }
            }
            catch (e) {
        ...
        _changed: function (event) {
            ...
            //Added by Fadi Omar
            if (this.options.syncFieldImageURL) {
                $(this.options.syncFieldImageURL).val(this.toImageURL());
            }
        ...
        /* Added by Fadi Omar 
            return Captured signature to image base64*/
        toImageURL: function () {
            var imageURL = '';
            if (this.ctx.canvas)
                imageURL = this.ctx.canvas.toDataURL();
            return imageURL;
        },

        /* Added by Fadi Omar 
            set Captured signature from image base64*/
        fromImageURL: function (imageURL, init) {
