...
qq.FineUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        text: {
            ...
            uploadButtonClass: 'qq-upload-button-default',
            cancelButton: 'Cancel',
            retryButton: 'Retry',
            deleteButton: 'Delete',
        ...
        template: '<div class="qq-uploader">' +
            ...
            //(!this._options.button ? '<div class="qq-upload-button btn"><div>{uploadButtonText}</div></div>' : '') +
             (!this._options.button ? '<div class="qq-upload-button {uploadButtonClass}"><div>{uploadButtonText}</div></div><i>&nbsp;&nbsp;{HintText}</i>' : '') +
    ...
    if (!qq.supportedFeatures.uploading || (this._options.cors.expected && !qq.supportedFeatures.uploadCors)) {
        ...
        this._options.template = this._options.template.replace(/\{uploadButtonClass\}/g, this._options.text.uploadButtonClass);
        this._options.template = this._options.template.replace(/\{HintText\}/g, this._options.text.HintText);
...
qq.extend(qq.FineUploader.prototype, {
    ...
    _onDeleteComplete: function(id, xhr, isError) {
        ...
        var item = this.getItemByFileId(id), spinnerEl, statusTextEl;
        if (item) {
            spinnerEl = this._find(item, 'spinner');
            statusTextEl = this._find(item, 'statusText');
            qq(spinnerEl).hide();
    ...
    _sendDeleteRequest: function (id) {
        ...
        var item = this.getItemByFileId(id), deleteLink, statusTextEl;
        if (item) {
            deleteLink = this._find(item, 'deleteButton');
            statusTextEl = this._find(item, 'statusText');
            qq(deleteLink).hide();
    ...
    //created by Fadi Omar to prepare file loaded uploader
    loadDumpFile: function (fileName) {
        this._handler.addDump(fileName);
        this._addToList(0, fileName);
    }
});
